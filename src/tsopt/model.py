...
class Model:
    ...
    def stage_edges(self):
        return [
            [[(i, o) for o in outs] for i in inps] for inps, outs in self.stage_nodes
        ]

    def get_abbrev(self, layer):
        return "".join([s[0].upper() for s in re.split(r"[ -\._]", layer)])
    ...
    def get_nodes(self, idx) -> list:
        ...
        return [f"{abbrev}{n}" for n in range(0, length)]

    @property
    def nodes(self):
        return [self.get_nodes(i) for i in range(len(self))]
    ...
    def set_layers(self, new: list):
        ...
        if diff > 0:
            self._shape += [1 for _ in range(diff)]
        elif diff < 0:
            self._shape = self._shape[:diff]
    ...
    def _validate_layer_names(self, layers):
        ...
        assert [
            not s[0].isdigit() for s in abbrevs
    ...
    def template_stages(self, fill=np.nan):
        # formerly edges
        return [
            pd.DataFrame(fill, index=idx, columns=cols)
            for idx, cols in staged(self.nodes)
        ]

    def template_layers(self, fill=np.nan):
        # formerly vectors
        return [NodeSR(fill, index=nodes) for nodes in self.nodes]
    ...
    @property
    def node(self):
        return self.con.node

    @property
    def edge(self):
        return self.con.edge
    ...
    def set_node_constraints(self):
        ...
        """
        sum_func = (
            lambda i, node: self.sum_outflow(i, node)
            if i == 0
            else self.sum_inflow(i, node)
        )
        ...
        for ins, outs in self.stage_nodes:
            for node in ins:
                setattr(
                    ...
                    node,
                    pl.LpVariable.dicts(
                        node, list(outs), lowBound=0, upBound=None, cat="Integer"
                    ),
                )
