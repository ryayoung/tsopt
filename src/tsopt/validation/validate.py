...
def validate(mod) -> None:
    ...
    def validate_edge_constraint(self, stage, df_new, sign, force=False) -> bool:
        ...
        def edge_bounds_valid(bounds: dict) -> None:
            ...
            bad_edges = [
                "-".join(pair) for pair in df_diff[df_diff < 0].stack().index.tolist()
            ]
            if len(bad_edges) == 0:
                return
    ...
    def validate_min_edge_constraint(self, edges: pd.DataFrame, stage: int) -> None:
        node_bounds = [
            self.dv.templates.layer_bounds()[stage],
            self.dv.templates.layer_bounds()[stage + 1],
        ]
        ...
        def sum_exceeds_flow_capacity() -> None:
            if edges.values.sum() > self.flow_capacity:
                raise InfeasibleEdgeConstraint(
                    f"Lower bounds on edges between {layers[0]} and {layers[1]} are "
        ...
        def edges_within_node_capacity() -> None:
            ...
            bad_nodes = [
                [n for n in dem.index if cap[n] < dem[n]]
            ...
            for nodes, layrs in zip(bad_nodes, (layers, layers[::-1])):
                if len(nodes) == 1:
                    raise InfeasibleEdgeConstraint(
                        f"The sum of the lower bounds on {layrs[0]} {nodes[0]}'s edges "
                        f"with {txt.plural(layrs[1])} exceed {nodes[0]}'s capacity."
                    )

                elif len(nodes) > 1:
                    raise InfeasibleEdgeConstraint(
                        f"Lower bounds on edges that connect {txt.plural(layrs[0])} "
        ...
        def inc_layer_demand_under_flow_cap() -> None:
            ...
            nodes_over = [
                list((old < new).index[old < new]) for new, old in zip(added, old)
            ]

            for new, nodes, layrs in zip(
                new_demand, nodes_over, (layers, layers[::-1])
            ):
                if new.sum() > self.flow_capacity:
                    outro = (
                        f"Total {layrs[0]} demand is increased to {new.sum()} units. "
    ...
    def validate_max_edge_constraint(self, edges: pd.DataFrame, stage: int) -> None:
        ...
        def edges_meet_node_demand() -> None:
            ...
            bad = [
                [node for node in cap.index if dem[node] > cap[node]]
                for cap, dem in zip(capacities, demands)
            ]
            for nodes, layrs in zip(bad, (layers, layers[::-1])):
                if len(nodes) == 1:
                    ...
                    raise InfeasibleEdgeConstraint(
                        f"Upper bounds were placed on all edges connecting node(s) "
        ...
        def overflow_exceeds_cross_capacities() -> None:
                ...
                [
                    {
                        ...
                        "from": cap[
                            cap.index.isin(
                                [c for c in df.columns if pd.isna(df.loc[k, c])]
                            )
                        ],
                    }
            ...
            cross_req = [
                [dict(item, **{"avail": item["from"].sum()}) for item in layr]
                for layr in cross_req
            ]
            cross_req_invalid = [
                [i for i in layr if i["avail"] < i["req"]] for layr in cross_req
            ]
            ...
            assignments = [
                list(set([tuple(node["from"].index) for node in layr]))
                for layr in cross_req
            ]
            assignments = [
                {
                    nodes: {
                        "sum_req": sum(
                            [
                                node["req"]
                                for node in cross_req[i]
                                if tuple(node["from"].index) == nodes
                            ]
                        ),
                        "by": tuple(
                            [
            ...
            assignments_invalid = [
                {k: v for k, v in layr.items() if v["sum_req"] > v["avail"]}
                for layr in assignments
            ]
